...
const BANCOS_SUPORTADOS = ['748', '237', '001', '461', '756'];
                ...
                <tr key={b.id} className="hover:bg-blue-50/50 cursor-pointer" onClick={() => onSelecionar(b)}>
                  ...
                  <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                    <div className="flex gap-1">
                      ...
                      {temNossoNumero && (
                        <a
                          href={`/${window.location.pathname.split('/').filter(Boolean)[0] || 'boleto'}/api/autoriza/${b.id}/pdf`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="p-1.5 text-gray-500 hover:text-green-600 hover:bg-green-50 rounded-md inline-flex"
