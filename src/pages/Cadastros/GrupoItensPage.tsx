...
import { GrupoItem } from 'shared/components/Cadastros/GrupoItemTypes';
import GrupoItensForm from 'shared/components/Cadastros/GrupoItensForm';
...
const GrupoItensPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<GrupoItem>>({});
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/grupos-itens?${params}`);
      ...
      console.error('[GrupoItensPage] Erro ao carregar:', error);
    } finally {
      setLoading(false);
    }
  }, [currentPage, search]);
  ...
  const handleEditar = (grupo: GrupoItem) => {
    setFormData(grupo);
    setModo('editar');
    setShowModal(true);
  };

  const handleExcluir = async (grupo: GrupoItem) => {
    if (!window.confirm(`Excluir grupo "${grupo.descr_gru}"?`)) {
      return;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/grupos-itens/${grupo.grupo_gru}`, {
        method: 'DELETE',
      });
      ...
      console.error('[GrupoItensPage] Erro ao excluir:', error);
      alert('Erro ao excluir');
    }
  };
  ...
  const handleGravar = async () => {
    try {
      const url = modo === 'incluir'
        ? `${API_BASE_URL}/grupos-itens`
        : `${API_BASE_URL}/grupos-itens/${formData.grupo_gru}`;
      ...
      if (data.success) {
        alert(`Grupo ${modo === 'incluir' ? 'criado' : 'atualizado'} com sucesso!`);
        setShowModal(false);
        setModo('localizar');
        carregarDados();
      } else {
      ...
      console.error('[GrupoItensPage] Erro ao gravar:', error);
  ...
  const handleFormChange = (campo: keyof GrupoItem, valor: any) => {
  ...
  return (
    <Container>
      <Header>
        <Title>
          <FontAwesomeIcon icon={faFolder} color="#2563eb" />
          Cadastro de Grupos de Itens
        </Title>
      ...
      {showModal && (
        <ModalOverlay onClick={(e) => e.target === e.currentTarget && handleCancelar()}>
          <ModalContent>
            <ModalHeader>
              <ModalTitle>
                {modo === 'editar' ? '✏️ Editar Grupo de Itens' : '➕ Novo Grupo de Itens'}
              </ModalTitle>
