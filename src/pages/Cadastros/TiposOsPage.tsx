...
import { TipoOs } from 'shared/components/Cadastros/TipoOsTypes';
import TiposOsForm from 'shared/components/Cadastros/TiposOsForm';
...
const TiposOsPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<TipoOs>>({});
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/tipos-os?${params}`);
      ...
      console.error('[TiposOsPage] Erro ao carregar:', error);
    } finally {
      setLoading(false);
    }
  }, [currentPage, search]);
  ...
  const handleEditar = (tipo: TipoOs) => {
    setFormData(tipo);
    setModo('editar');
    setShowModal(true);
  };

  const handleExcluir = async (tipo: TipoOs) => {
    if (!window.confirm(`Excluir tipo de O.S. "${tipo.descr_os}"?`)) {
      return;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/tipos-os/${tipo.codigo_os}`, {
        method: 'DELETE',
      });

      if (response.ok) {
        alert('Tipo de O.S. excluído com sucesso!');
        carregarDados();
      } else {
        alert('Erro ao excluir');
      }
    } catch (error) {
      console.error('[TiposOsPage] Erro ao excluir:', error);
      alert('Erro ao excluir');
    }
  };
  ...
  const handleGravar = async () => {
    try {
      const url = modo === 'incluir'
        ? `${API_BASE_URL}/tipos-os`
        : `${API_BASE_URL}/tipos-os/${formData.codigo_os}`;
      ...
      if (data.success) {
        alert(`Tipo de O.S. ${modo === 'incluir' ? 'criado' : 'atualizado'} com sucesso!`);
        setShowModal(false);
        setModo('localizar');
        carregarDados();
      } else {
      ...
      console.error('[TiposOsPage] Erro ao gravar:', error);
  ...
  const handleFormChange = (campo: keyof TipoOs, valor: any) => {
  ...
  return (
    <Container>
      <Header>
        <Title>
          <FontAwesomeIcon icon={faWrench} color="#2563eb" />
      ...
      {showModal && (
        <ModalOverlay onClick={(e) => e.target === e.currentTarget && handleCancelar()}>
          <ModalContent>
            <ModalHeader>
              <ModalTitle>
                {modo === 'editar' ? '✏️ Editar Tipo de O.S.' : '➕ Novo Tipo de O.S.'}
              </ModalTitle>
